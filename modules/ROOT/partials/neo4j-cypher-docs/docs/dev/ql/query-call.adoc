...
For more information on how to determine the input parameters for the `CALL` procedure and the output parameters for the `YIELD` procedure, see <<call-view-the-signature-for-a-procedure>>.
...
For a list of these, see <<operations-manual#neo4j-procedures, Operations Manual -> Procedures>>.
...
See <<cypher-parameters>>, for more about querying with parameters
