...
`ORDER BY` relies on comparisons to sort the output, see <<cypher-ordering, Ordering and comparison of values>>.
...
----
  N0 [
    label = "name = \'A\'\lage = 34\llength = 170\l"
  ]
  N0 -> N1 [
    color = "#2e3436"
    fontcolor = "#2e3436"
  ...
  N2 [
    label = "name = \'C\'\lage = 32\llength = 185\l"
  ]
...
For details, see <<property-types-sip-note>>.
