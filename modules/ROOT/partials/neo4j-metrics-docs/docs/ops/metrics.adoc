...
NOTE: Metrics reporting is only available in Neo4j Enterprise Edition.
...
For Graphite integration add the following settings to _neo4j.conf:_
...
For storing metrics in local CSV files add the following settings to _neo4j.conf:_
...
include::available-metrics.adoc[]
