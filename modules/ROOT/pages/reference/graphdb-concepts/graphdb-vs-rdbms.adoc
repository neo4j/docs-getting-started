...
:description: Learn more about the transition from a relational to a graph database, from a developer's point of view.

Migrating from a relational to a graph database requires an understanding of the conceptual differences and similarities between both technologies.
This includes how each kind of data model and query language works, and which are more suitable for a project.

[#relational-vs-graph]
== Relational databases: an overview

Relational databases have been a core technology for software applications since the 1980s, and it continues to be relevant to this day.
It is characterized by its method of storing highly-structure data in tables with predetermined columns and rows of defined types of information.

Despite its many advantages, a relational database has a very strict method of data organization which can be costly, from a computing perspective.
In it, references to other rows and tables are indicated with primary key attributes via foreign key columns.
...
These operations are compute-heavy and memory-intensive and have an exponential cost.
 ...
 that can be heavy on computing and memory processing, thus having an exponential cost.
 can also be compute and memory-intensive, and consequently have an exponential cost.
