...
Also note that when you run Cypher code in the query pane, it always creates a new pane with the results below it.

== Find actors and movies

=== A specific actor

If you want to find a specific actor in the graph, you can `MATCH` the `Person` node and refer to the actor using the `name` property with the `WHERE` clause:
...
To list the movies Tom Hanks took part of, you need to query his `Person` node and how it is connected to the `Movie` nodes through a relationship.
...
But, to understand better how it is built, it can be split first with the part of the query that aims to find what movies Tom Hanks has acted in:
...
Then, combine with people who acted in those same movies:
...
In this query, the relationship is undirected, because you want to retrieve all relationships between these two nodes, no matter the direction.
...
[source, cypher]
--
MATCH (bacon:Person {name:"Kevin Bacon"})-[*1..3]-(hollywood:Person|Movie)
RETURN DISTINCT bacon, hollywood
--

Note that the variable `hollywood` includes two different types of nodes: `Person` and `Movie`.
This is something you can do with Cypher, in case you want to retrieve information from different node types, but use only one variable.

The reason why `RETURN` includes `DISTINCT` now is so that nodes don't get duplicated in the results.
This could happen because Kevin Bacon could have worked with a same actor in different movies, thus causing the duplication.
...
[source, cypher]
--
MATCH p = (bacon:Person {name:"Kevin Bacon"})-[*1..3]-(hollywood:Person|Movie)
RETURN DISTINCT bacon, hollywood, p
--
...
If you want to find what is the shortest path between Kevin Bacon and Meg Ryan, you can write a query like this:
...
To learn how to remove all properties, see link:https://neo4j.com/docs/cypher-manual/current/clauses/remove/#remove-remove-all-properties[Cypher -> `REMOVE`].
