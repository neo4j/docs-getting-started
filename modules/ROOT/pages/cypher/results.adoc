= Refining results
:description: This guide describes how to manipulate the output of Cypher queries in order to get the refined results.
...
To return only results that contain a value, you can instead use the `WHERE` subclause and specify that it returns only results that are not `null`:
...
[IMPORTANT]
====
You need to include `CYPHER 25` in the beginning of your query in case you are using a previous Neo4j version.
...
The `STARTS WITH` operator searches for property values that begin with a string you specify, for examplle, the letter "M":
...
The `CONTAINS` operator searches for a specified string which is part of a property value, for example, if it contains the letter "A":
...
The `ENDS_WITH` operator searches for a specified value at the end of a property value, for example, a value that ends with "N": the end of the property string for the value you specify.
...
With the `IN` operator, you can specify an array of values and validate a property, for example, if any `Person` node in the graph has "1", "5" or "6" values for `yearsExperience`:
...
For example, if you want to find which of Jennifer's friends do not work for any company, you can do it like this:
...
And you get only "Mark" as the result.

==== `OPTIONAL` patterns

There are cases in which you might want to retrieve results from patterns, even if they do not match the entire pattern or all criteria.
...
Since you used `OPTIONAL MATCH`, his `Person` node is still returned from the first match, but the second match is not found, so it returns `null`.

To see the difference, try running the query without the `OPTIONAL` in front of the second match.
Joe's row no longer returns in the result.
...
You can use queries with patterns that have more than one single relationship.
For example, if you want to know who else likes graphs besides Jennifer, you can write the query like this:
...
Another possibility is trying who of these people who like graphs is also friends with Jennifer:
...
You can use comparisons to retrieve results, starting with equality: 
...
In this case, only the second condition matches the graph pattern `(p:Person)-[r:ACTED_IN]->(m:Movie)`, therefore the output is the following:
...
You can also use patterns as predicates for more advanced customization of results.
While the `MATCH` clause *expands* the number and shape of patterns matched, a pattern predicate *restricts* the result.

This can be achieved by appending `NOT` to the `WHERE` clause, so that it only allows paths that do *not* satisfy the specific pattern to pass:
...
This is the case because the query focuses on `Person` nodes that are connected to `Movies` nodes only through the `DIRECTED` relationship, but not `ACTED_IN`.
...
You can query data within a certain range in number or date, for example, if you want to check for events within a certain timeline, age values, etc.
...
This is the query you can use when you want to know who has a range of three to seven years of experience:
...
However, in many in cases, you may want to use an alias to make the results more understandable by using `AS`.

For example, if you want to have the tabular result with a column titled `name` instead of the label `p.name`, you can write the query like this:
...
| (:Person {twitter: "@melissa", name: "Melissa"})
| "Melissa"

| (:Person {twitter: "@dan", name: "Dan", yearsExperience: 6})
| "Dan"

| (:Person {name: "Sally", yearsExperience: 4})
| "Sally"

| (:Person {name: "John", yearsExperience: 5})
| "John"
...
This is because she is connected with other people through two `IS_FRIENDS_WITH` relationships.
To avoid this duplication, you can add `DETACH` after `RETURN`:
...
For example, to list how many years of experience each person has in decrescent order, you can write the query like this:
...
This means the property `yearsExperience` doesn't exist to all `Person` nodes, but existing entries are ordered in decrescent order, as stated in the query.

Optionally, you could avoid returning `null` values by using `WHERE` and `IS NOT NULL`:
...
When you don't want to retrieve every entry, but limit the result by a number, you can use `LIMIT` after `RETURN`.
...
You can use them with the `RETURN` clause to aggregate results.
...
For example, if you want to see which people in the example graph like "Graphs" and/or "Query Languages", you can write the following query:
...
If you have a list of values, you can also find the number of items in that list or calculate the size of an expression using the link:https://neo4j.com/docs/cypher-manual/current/functions/scalar/#functions-size[`size()`] function.

For example, if you want to know how many friends each person in the graph has, you can write the following query:
...
Note that if you put a direction to the relationship in the first line of the query:
