...
[IMPORTANT]
====
If you are using any other Cypher version, you need to add `CYPHER 25` to the first line of the query.
...
Alternatively, you can also filter the result by finding what is not included to a given pattern.
...
For example, if you want to know who works for the same company and are also friends:
...
Notice that the query has a comma at the end of the first line, and another pattern is added to `MATCH` on the next line.
...
For example, to know which people are connected through a `IS_FRIENDS_WITH` relationship:
...
Note that all names appear more than once.
...
Cypher provides several link:https://neo4j.com/docs/cypher-manual/current/functions/aggregating/[aggregating functions] that allow calculations over a set of values to return a single value.
...
If you have a list that you want to inspect or separate the values, Cypher offers the link:https://neo4j.com/docs/cypher-manual/current/clauses/unwind/[`UNWIND`] function for that.
...
Note that the results include repetitions that can be avoided if you use the link:https://neo4j.com/docs/cypher-manual/current/functions/aggregating/#functions-collect[`collect()`] function.
