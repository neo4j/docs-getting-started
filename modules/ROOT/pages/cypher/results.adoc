...
In this case, out of the 12 `Person` nodes in the graph, only four (Melissa, Dan, Jennifer, Mark) contain a value for the `twitter` property, so besides them, you also retrieve another 8 `null` results.
...
For example, if you want to find any names that include "Jo" and be case sensitive (which is the default):
...
--
MATCH (p:Person)
WHERE toLower(n.name) CONTAINS "jo"
RETURN p.name
--

Or `toUpper()` if you want to query a value in uppercase:

[source, cypher]
--
MATCH (p:Person)
WHERE toUpper(n.name) CONTAINS "JO"
RETURN p.name
--
...
This allows you to chain different patterns together, similary to when you used xref:cypher/results.adoc#_patterns[`WHERE exists(<pattern>)`] previously.
...
Since you didn't use xref:cypher/results.adoc#_order[`ORDER BY`] to specify an order to the results, the query retrieves the first three `Person` nodes to be added to the graph.
...
For example, Jennifer is connected to Mark through a `IS_FRIENDS_WITH` relationship that comes from Jennifer to Mark (i.e. `(jennifer)-[:IS_FRIENDS_WITH]->(mark)`), not `(mark)-[:IS_FRIENDS_WITH]->(jennifer)`.
