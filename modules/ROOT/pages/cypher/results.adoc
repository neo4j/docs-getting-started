...
Alternatively, you can also filter the result by excluding certain patterns from the results:
...
This allows you to chain different patterns together, similar to when you used xref:cypher/results.adoc#filter-patterns[`WHERE exists(<pattern>)`] previously.
...
For more information on how to query for patterns, see link:https://neo4j.com/docs/cypher-manual/current/patterns/[Cypher -> Patterns].
...
Cypher provides several link:https://neo4j.com/docs/cypher-manual/current/functions/aggregating/[aggregating functions] that let you calculate a single value from a set of values.
...
You can use the link:https://neo4j.com/docs/cypher-manual/current/functions/aggregating/#functions-collect[`collect()`] function to group results together.
