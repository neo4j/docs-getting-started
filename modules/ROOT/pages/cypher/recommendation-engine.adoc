...
:description: Learn how to build a recommendation engine with Cypher.
:page-aliases: ROOT:guide-build-a-recommendation-engine.adoc

This tutorial uses the xref:appendix/example-data.adoc[Movie dataset] to show how to build two recommendation engines:

* Who an actor hasn't worked with and should work next
* Who can introduce the actors to each other

If you are completely new to Cypher, see the tutorial xref:cypher/intro-tutorial.adoc[Get started with Cypher] to learn how to run basic queries against the same example dataset used here.
...
Copy and paste the following Cypher to the Cypher editor and execute the query:
...
You now have a graph with 171 nodes and 253 relationships.

== Recommendations

The Movie dataset includes information about actors, directors, and movies they have directed and/or acted in.

The type of recommendation you can retrieve is based not only on the information that exists in the graph (e.g., what year someone was born), but also how each data point interacts with the others (e.g., what movies have actors that were only born after 1900).

Each degree of complexity of a recommendation requires the query to be modified accordingly.
Before building the two recommendation engines, you can try simple filtering queries to explore the graph.

=== Simple filtering

If you want to watch only movies which Keanu Reeves has acted in, you can find them with this query:
...
The premise of this simple filtering example is using specific information that can be found in properties.
You can apply the same logic to find, for example, in which movie Keanu Reeves played the role "Julian Mercer" by adding the property `role` to the relationship `:ACTED_IN` in the query:

[source,cypher]
--
MATCH (p:Person {name:"Keanu Reeves"})-[:ACTED_IN {roles:['Julian Mercer']}]->(m:Movie)
RETURN m as Movies
--

You get the movie "Something's Gotta Give" as the result.

However, you can get more in-depth recommendations from the same dataset by applying more complex filters that capture multiple connections between the datapoints.

=== Finding second-degree co-actors

One way to take the previous recommendation query to the next level is by trying to find out what actors have worked with co-actors that worked with Keanu Reeves in his movies (i.e., second-degree co-actors in Keanu's network).

This query shows all actors Keanu hasn't worked with yet, and it makes sure not to return anyone who hasn't directly acted with him:

[source,cypher]
--
MATCH (keanu:Person {name:"Keanu Reeves"})-[:ACTED_IN]->(movie1:Movie)<-[:ACTED_IN]-(coActor:Person)-[:ACTED_IN]->(movie2:Movie)<-[:ACTED_IN]-(2dCoActor:Person)
WHERE keanu <> 2dCoActor
AND NOT (keanu)-[:ACTED_IN]->(:Movie)<-[:ACTED_IN]-(2dCoActor)
RETURN 2dCoActor.name
--
...
Note that a few names appear multiple times.
This is because there are multiple paths to follow from Keanu Reeves to these actors.

To see which second-degree co-actors appear most often in Keanu's network, you can take the frequency of occurrences into account by counting the number of paths between Keanu Reeves and each `2dCoActor` and ordering them by highest to lowest value:
...
WHERE keanu <> 2dCoActor
AND NOT (keanu)-[:ACTED_IN]->(:Movie)<-[:ACTED_IN]-(2dCoActor)
RETURN 2dCoActor.name, count(2dCoActor) as frequency
...
One of Keanu Reeves' second degree co-actor is Tom Hanks.
You can fine-tune the last query to see which movies and actors are between Keanu and Tom and find out who can introduce them:

[source,cypher]
--
MATCH (keanu:Person {name:"Keanu Reeves"})-[r:ACTED_IN]->(movie1:Movie)<-[r2:ACTED_IN]-(coActor:Person)-[r3:ACTED_IN]->(movie2:Movie)<-[r4:ACTED_IN]-(tom:Person {name:"Tom Hanks"})
WHERE NOT (keanu)-[r]->(:Movie)<-[r2]-(tom)
RETURN keanu, movie1, coActor, movie2, tom, r, r2, r3, r4
--

The graph result is:

image::cocoactors.svg[Graph results of querying showing which movies and actors are between Keanu Reeves and Tom Hank, role="popup-link",width=600]

Now you know that the actors Hugo Weaving and Charlize Theron could be the ones to connect Keanu Reeves and Tom Hanks, as they have worked with both of them in different movies.

A similar query is shown in the tutorial xref:cypher/intro-tutorial.adoc#_movies_and_actors_connected_to_kevin_bacon[Get started with Cypher], featuring the "Six degrees of Kevin Bacon" game.
In both cases, you can see in the graph how many hops exist between people.

== Conclusion

With these Cypher statements, you were able to query actors and movies from the database, and build recommendation engines that show:

* Who an actor hasn't worked with yet and could do next
* Who can introduce the actors to each other

== Keep learning

You can find other examples of how to build a recommendation engine using Cypher in these links:

* link:https://medium.com/neo4j/whats-cooking-approaches-for-importing-bbc-goodfood-information-into-neo4j-64a481906172[Recipe and food recommendations] -> read a blog post showing how to build a recommendation engine using the BBC goodfood recipes dataset.
* link:https://www.youtube.com/channel/UCvze3hU6OZBkB1vkhH2lH9Q/search?query=recommendation[Building recommendation engines] -> watch videos and lectures showing how to create a recommendation engine using Neo4j.
* link:https://neo4j.com/use-cases/real-time-recommendation-engine/[Recommendation use cases] -> see a list of use cases of recommendation engines built with Cypher.