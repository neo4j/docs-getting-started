...
[source,cypher]
--
MATCH (keanu:Person {name:"Keanu Reeves"})-[:ACTED_IN]->(movie1:Movie)<-[:ACTED_IN]-(coActor:Person)-[:ACTED_IN]->(movie2:Movie)<-[:ACTED_IN]-(coCoActor:Person)
WHERE keanu <> coCoActor
AND NOT (keanu)-[:ACTED_IN]->(:Movie)<-[:ACTED_IN]-(coCoActor)
...
To see which co-co-actors appear most often in Keanu's network, you can take the frequency of occurrences into account by counting the number of paths between Tom Hanks and each `coCoActor` and ordering them by highest to lowest value:
...
One of Keanu Reeves' "co-co-actors" is Tom Hanks.
