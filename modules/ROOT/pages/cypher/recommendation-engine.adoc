...
However, you can get deeper recommendations from the same dataset by applying more complex filters that address multiple connections between the datapoints.
...
This query shows all actors Keanu may not have worked with yet and it makes sure not to return anyone who hasn't directly acted with him:
...
* Who an actor hasn't worked with yet and should do next
* Who can introduce the actors to each other
