...
The type of recommendation you can retrieve is based not only on the information that exists in the graph (e.g., what year someone was born), but also how each datapoint interact with each other (e.g., what movies have actors that were only born after 1900).
...
If you want to watch only movies in which Keanu Reeves has acted in, you can find the information with this query:
...
And you get the movie "Something's Gotta Give" as the result.
...
One way to take the previous recommendation query to the next level is by trying to find out what actors have worked with co-actors that worked with Keanu Reeves in his movies (i.e., second-degree actors in Keanu's network).
...
One of those "co-co-actors" is Tom Hanks.

=== Exploring the paths between actors and movies

After the last query, you can now fine-tune it to see which movies and actors are between Keanu and Tom and find out who can introduce them:
...
A similar query was shown in the tutorial xref:cypher/intro-tutorial.adoc#_movies_and_actors_connected_to_kevin_bacon[Get started with Cypher], featuring the "Six degrees of Kevin Bacon" game.
