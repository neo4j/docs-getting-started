...
When using undirected relationships in a query, Cypher retrieves the relationship with all its connected nodes, no matter the direction of their connection. This allows queries to be flexible and not force the user to know the direction of the relationship stored in the database.

Undirected relationships are particularly useful when you don't know in what direction a relationship is stored in the database.
If you write a query with the wrong direction, Cypher will not return anything.
...
Previous examples feature code blocks containing after `//`.
