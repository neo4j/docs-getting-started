...
CSV data can be imported using either xref:data-import/csv-import.adoc[`LOAD CSV` clause in Cypher] or https://neo4j.com/docs/operations-manual/current/tutorial/neo4j-admin-import/[`neo4j-admin database import` for initial bulk imports^] of large datasets.

For loading JSON, XML, XLS you need to have the https://neo4j.com/docs/apoc/current/[APOC^] utility library installed, which comes with a https://neo4j.com/docs/apoc/current/import/[number of procedures for importing data^] also from other databases.
...
2. Then you can import the file using the `./bin/neo4j-admin database load  --overwrite-destination true --from file.dump` command.
3. Start the Neo4j server.

.Enterprise Edition (also Neo4j Desktop)
1. Import the file using the `./bin/neo4j-admin database load  --overwrite-destination true --from file.dump --database <dbname>` command.
...
For more details, see link:https://neo4j.com/docs/operations-manual/5/tools/neo4j-admin/migrate-database/[Operations manual -> Tools].
====
