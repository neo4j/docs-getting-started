...
image::graph_concept_three_nodes-arr.svg["Three blank nodes connected in a triangle by three directed relationships, illustrating the basic structure of a graph",width=300]
...
.Example graph
image::graph_simple-arr.svg["Example property graph: a Person and Actor node (Tom Hanks) connected by an ACTED_IN relationship to a Movie node (Forrest Gump), which a Person node (Robert Zemeckis) points to with a DIRECTED relationship",width=600]
...
.Node
image::graph_single_node-arr.svg["A single node with the labels Person and Actor and the properties name 'Tom Hanks' and born 1956, the simplest possible graph",width=150]
...
.Multiple labels
image::graphdb-simple-labels-multi-arr.svg["Three unconnected nodes showing multiple labels: a Person and Actor node (Tom Hanks), a Movie node (Forrest Gump), and a Person and Director node (Robert Zemeckis)",role="middle", width=500]
...
.Relationship
image::graph_example_relationship-arr.svg["Two nodes connected by a directed ACTED_IN relationship carrying the properties roles ['Forrest'] and performance 5",width=400]
...
image::graphdb-nodes-and-rel-self-arr.svg["A single Person and Actor node (Tom Hanks) with a KNOWS relationship pointing back to itself",role="middle", width=300]
...
.Relationship type
image::graphdb-nodes-and-rel-arr.svg["A Person and Actor node (Tom Hanks) with an outgoing ACTED_IN relationship to a Movie node (Forrest Gump)",role="middle", width=450]
...
image::graphdb-traversal-arr.svg["Traversal of the example graph: from the Tom Hanks node the ACTED_IN relationship to the Forrest Gump Movie node is highlighted, while the DIRECTED relationship from Robert Zemeckis is not visited",role="middle", width=400]

The traversal result could be returned as a path with the length `1`:

image::graphdb-path-arr.svg["The traversal result as a path of length one: the Tom Hanks node connected by an ACTED_IN relationship to the Forrest Gump Movie node",role="middle", width=400]
=====
...
.Path of length zero
image::graphdb-path-zero-arr.svg["A path of length zero: a single Person node (Tom Hanks) with no relationships",role="middle", width=150]


A path containing one relationship has the length of `1`.

.Path of length one
image::graphdb-path-example-loop-arr.svg["A path of length one: a single Person and Actor node (Tom Hanks) with one KNOWS relationship connecting the node to itself",role="middle", width=300]


[[graphdb-schema]]
== Schema

A xref:cypher/schema.adoc[schema] in Neo4j refers to indexes and constraints.
...
To see examples of how to work with indexes, see xref::cypher/schema.adoc#_using_indexes[Using indexes].
...
To see examples of how to work with constraints, see xref::cypher/schema.adoc#_using_constraints[Using constraints].
