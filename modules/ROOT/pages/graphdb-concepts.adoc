...
Labels are used to shape the domain by grouping (classify) nodes into sets where all nodes with a certain label belong to the same set.
...
[TIP]
====
For a thorough description of the available data types, refer to the xref:4.4-preview@cypher-manual:ROOT:syntax/values/index.adoc#cypher-values[Cypher manual -> Values and types].
====
