...
Neo4j uses xref:cypher/index.adoc[Cypher], a declarative query language similar to SQL, but optimized for graphs.
...
You can also xref:appendix/example-data.adoc[download] the example datasets and then import them to your instance.
...
* The link:{docs-home}/ogm-manual/current/[Object Graph Mapping (OGM)] library, maps nodes and relationships in the graph to objects and references in a domain model.
...
* The link:{docs-home}/query-api/current/[Neo4j Query API] allows you to develop client applications in languages not currently supported by Neo4j.
...
To learn more about xref:graph-database.adoc[what a graph database is] and xref:appendix/graphdb-concepts/index.adoc[the concepts] behind the technology, continue reading the documentation or browse xref:appendix/getting-started-resources.adoc[other curated resources].
