...
However, adding new data poses the risk of duplication, which, in turn, affects the xref:data-modeling/tutorial-refactoring.adoc#_performance_check[performance of your graph].
...
** Example: In the xref:data-modeling/tutorial-refactoring.adoc#_remaining_or_new_use_cases[previous step], you refactored the property 'languages' to become a node to avoid having the entry "English" duplicated on all `Movie` nodes.
