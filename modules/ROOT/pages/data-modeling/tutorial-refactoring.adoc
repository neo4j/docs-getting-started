...
:page-aliases: data-modeling/graph-model-refactoring.adoc
...
However, adding new data poses the risk of duplication, which, in turn, affects the xref:#_performance_check[performance of your graph].
...
** Example: In the xref:#_eliminating_duplicated_data[previous step], you refactored the property 'languages' to become a node to avoid having the entry "English" duplicated on all `Movie` nodes.
