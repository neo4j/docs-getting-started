...
In the case of graph data models, they are often referred as being _whiteboard-friendly_.
...
A similar process also exists in graph data modeling.
However, instead of adapting the <<data model>> to a normalized table structure, the graph data model stays the same.
Take this example using data about the movie "The Matrix", found in the xref:appendix/example-data.adoc[*Movies* example dataset]:
...
This helps differentiating entities and thus making it easier to retrieve them from the database:

.Matrix - add labels and properties
image::matrix-whiteboard-model3.svg[500,500,role="popup-link"]

To visualize that in Neo4j Browser or in Query, you can retrieve the information from the xref:appendix/example-data.adoc[*Movies* example dataset] or write the data from scratch:
...
To retrieve this information from the database, write the following query:
...
[TIP]
====
If you are new to these concepts, refer to xref:cypher.adoc[What is Cypher] for an introduction.
====

=== Domain definition

It is important to understand the domain of your dataset when designing data models.
...
Neo4j is *schema-free*, which means that the data model can adapt and change easily as a project advances, thus making refactoring and versioning simpler processes.
...
Still, since datasets can often share many points in common (e.g. domains, objects, measurements), you can opt to follow strategies that are already known and used in the relevant industry.
