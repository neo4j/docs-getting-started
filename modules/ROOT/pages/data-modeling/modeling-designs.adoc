...
:description: This page features examples of graph data modeling patterns and designs which are commonly used with Neo4j.

This page features examples of graph data modeling patterns and designs which are commonly used with Neo4j.
The purpose is to get an overview of the options available for building (graphy) data models based and how some known strategies can be adapted to your own project.
...
For example, here there is a person who works at a company and there is information about their position:
...
In a mathematical graph, you could use the same relationship `WORKED_AT` to connect the `Person` node with both `Position` and `Company` nodes.
However, in Neo4j, you can't do it.

Instead, you could either turn the `Position` into a property of the `WORKED_AT` relationship or use an intermediate node between the `Person` and `Company` nodes:

image::refactored-hyperedge.svg[Instead of using one single relationship to both Company and Position nodes, an intermediary Employment node can solve the problem of incompatibility of hyperedges in Neo4j,width=500,role=popup]
...
The previous example can be expanded to show more people who have been employees of the Acme company and thus represent a share context between multiple nodes using a common event (employment):
