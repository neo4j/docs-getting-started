...
Now, instead, if we were to model our movies and genres as separate nodes and create a relationship between them, we would come up with a model like the _Figure 2_.

.Graph of movies and their genres
...
We might start with a model like the _Figure 3_ below to show how flights travel from airport to airport.

.Graph model of airline flights
...
.Graph model of airline flights after review
image::modeling_airport_flight_dates-arr.svg[role="popup-link",600,600]

For the full walkthrough of the modeling process for airline flights, see link:https://maxdemarzi.com/2015/08/26/modeling-airline-flights-in-neo4j/[Max's blog post^].
