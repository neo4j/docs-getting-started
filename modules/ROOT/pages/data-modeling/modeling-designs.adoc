...
In this data structure, a simple-linked list is where each node links to the next node only:
...
Double-linked lists are not recommended because relationships become redundant (if one is the next, then the other is the previous).
