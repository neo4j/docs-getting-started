...
Neo4j doesn't support hyperedges, but instead it has the option to create intermediate nodes.
These are often created to model relationships that exist between more than two entities.
...
The way Peter at Marvel handled intemediate nodes in their data is by creating an `Appearance` node that represents the intersection of a `Person` and an `Alias` at a particular time.
