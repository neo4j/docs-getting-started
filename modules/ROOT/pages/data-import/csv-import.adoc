[[csv-import]]
= TImport CSV data using `LOAD CSV`
:description: This guide shows how to use the command LOAD CSV to import small to medium datasets in CSV format into Neo4j using Aura and on-premise deployments.
:page-pagination:
...
This guide shows how to import CSV data into Neo4j using link:https://neo4j.com/docs/cypher-manual/current/clauses/load-csv/[`LOAD CSV`] command.
...
However, in this guide, you will use a link to load the CSV file:
