...
2. The `neo4j-admin import` tool: command-line tool useful for straightforward loading of large data sets.
...
We provide a brief oview of the `LOAD CSV` Cypher command and `neo4j-admin import` command, how they operate, and how to get started with a general use case.
...
Our link:https://neo4j.com/docs/cypher-manual/4.4/[Cypher manual^] contains a page devoted to its usage.
It is widely applicable.
`LOAD CSV` is not just the basic data ingestion mechanism.
...
More information is in the Operations Manual on link:https://neo4j.com/docs/operations-manual/4.4/tools/cypher-shell/[Cypher shell^].
--
...
File locations for each OS are listed in our link:https://neo4j.com/docs/operations-manual/4.4/configuration/file-locations[Neo4j Operations Manual^].
It is recommended to put files in Neo4j's _import_ directory, as it keeps the environment secure.
However, if you need to access files in other locations, you can find out which setting to alter in our link:https://neo4j.com/docs/cypher-manual/4.4/clauses/load-csv/#query-load-csv-introduction[Cypher manual^].
Default import folder paths are shown in link:https://neo4j.com/developer/kb/import-csv-locations/[the Knowledge Base article^].
...
For more information, see the link:https://neo4j.com/docs/cypher-manual/4.4/query-tuning/using/#query-using-periodic-commit-hint/[manual page^] on `PERIODIC COMMIT`.
+
.Example
[source,cypher, role= nocopy noplay]
----
...
For more information, see link:https://neo4j.com/docs/cypher-manual/4.4/clauses/call-subquery/#subquery-call-in-transactions/[Cypher Manual on subqueries]. 
--
...
For more information about the `Eager` operator, see the link:https://neo4j.com/docs/cypher-manual/4.4/execution-plans/operators/#query-plan-eager[Cypher manual].
...
Usually, you can create or update 1M records in a single transaction per 2 GB of heap.
In `neo4j.conf`:
+
...
Suppose you want to import order data via `neo4j-admin import` into a Neo4j instance.
...
There is specific markup on specific columns, which will be explained.
...
For more details on this header format and the tool, see the section in the link:https://neo4j.com/docs/operations-manual/4.4/tools/neo4j-admin/neo4j-admin-import/[Neo4j Operations Manual^] and the accompanying link:https://neo4j.com/docs/operations-manual/4.4/tutorial/neo4j-admin-import/[tutorial^].
...
Documentation for proper escaping is in the link:https://neo4j.com/docs/cypher-manual/4.4/styleguide/#cypher-styleguide-meta-characters[Cypher style guide] and a link:https://neo4j.com/developer/kb/parsing-of-quotes-for-load-csv-and-or-import/[Knowledge Base article^].
