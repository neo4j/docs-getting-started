...
. The link:{docs-home}/operations-manual/current/tools/neo4j-admin/neo4j-admin-import/[`neo4j-admin database import`] command: the most efficient way for importing large CSV files into an empty database (full) or when import cannot be completed in a single full import, by allowing the import to be a series of smaller imports (incremental).
