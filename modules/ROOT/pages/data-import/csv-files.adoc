...
When working with CSV files as the format of your data to be imported into Neo4j, there are a few points to be considered so that you can better use the available tools.
This page addresses the details of a CSV file and other recommendations when importing them.

== Structure of a CSV file

When analyzing the content of a CSV files, there are several elements that can help you understand the data that you are importing and xref:data-modeling/index.adoc[modeling] later on.
...
See more information on how to manipulate data formats in the xref:#_converting_data_values[Converting data values] section.
...
When working with CSV files with no header row, you will need to know the order of the columns and refer to them through link:https://neo4j.com/docs/cypher-manual/current/indexes/[indexes].
...
If the entry `Pires, Gerard` wasn't between quotation marks, it would count as two different entries (i.e. one for `Pires` and another for `Gerard`).
...
If the source data is de-normalized, there will typically be a single CSV file which contains all the data, often duplicated where there are relationships between entities.
For example:
...
The first thing you need to consider when importing CSV data is the *source* of this file.
...
However, real-world data is messy, which means some values need to be cleaned up or transformed before you import it to another system.
...
This means you would need to remove irrelevant information _before_ you import the CSV file into Neo4j, thus making the process easier and more efficient in the long run.

. *Inconsistency between headers and data*
+
Headers can become inconsistent with data as they could be missing, or be lost in too many columns, or different delimiters are used.
...
* Adjust formatting, delimiters, columns, etc at this stage, so you don't need to spend more time later.
...
+
Computers do not handle well is inconsistent data.
...
If you have a field in the CSV that is a list of items that you want to split, you can use the Cypher `split()` function to separate arrays in a cell.
For example:
...
Note that Joe Smith and Trevor Scott have multiple skills.
...
For example, you can set the `businessType` property based on shortened value in CSV:
...
You can use the following third-party tools to make sure your CSV file is in good quality and that you can import data efficiently:
...
Most Neo4j's xref:data-import/index.adoc#_methods_comparison[import methods] are recommended for importing small or medium-sized datasets (up to 10 million records).
If you want to import larger datasets, see link:https://www.neo4j.com/docs/operations-manual/current/import/#import-tool-full[`neo4j-admin database import full`] and link:https://www.neo4j.com/docs/operations-manual/current/import/#import-tool-incremental[`neo4j-admin database import incremental`].
