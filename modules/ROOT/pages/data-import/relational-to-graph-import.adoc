...
* xref:data-import/csv-import.adoc[*LOAD CSV*]: possibly the simplest way to import data from your relational database.
Requires a dump of individual entity-tables and join-tables formatted as CSV files.

* link:https://neo4j.com/docs/data-importer/current/[*Data Importer*]: UI-based tool for importing data into Neo4j.
Available both as a standalone tool and as a service in Aura for importing CSV and TSV files.

* link:https://neo4j.com/docs/apoc/current/[*APOC*]: Awesome Procedures on Cypher.
...
* link:https://neo4j.com/labs/etl-tool/[*ETL Tool*]: Neo4j Labs UI tool that translates relational to graph from a JDBC connection.
Allows bulk data import for large data sets with a fast performance and simple user experience.

* link:https://medium.com/@samuel.second/apache-hop-connecting-to-neo4j-using-environments-e6839c279de0[*Apache Hop*]: open-source tool for enterprise-scale data export and import.
...
* Other ETL tools: there are also a few vendor and community tools available for similar etl processes and GUI interaction for getting data in various formats into and out of Neo4j.
Some of these tools also can map out the flow and transformation of data through the system.

* xref:languages-guides/index.adoc[Programmatic via drivers]: ability to retrieve data from a relational database (or other tabular structure) and use the bolt protocol to write it to Neo4j through one of the drivers with your programming language of choice.
...
Refer to link:https://neo4j.com/docs/data-importer/current/[Neo4j Data Importer] documentation for a complete guide on how ot import CSV and TSV files into Aura or using the tool as a stand-alone service.
