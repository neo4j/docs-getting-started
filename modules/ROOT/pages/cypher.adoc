...
Cypher is Neo4j's declarative, link:https://neo4j.com/docs/cypher-manual/current/appendix/gql-conformance/[GQL conformant] query language. 
...
For example, if you want to bound all nodes labeled `Person` to the variable `p`, you write `(p:Person)`.
Likewise, if you want to use a full word, then you can write `(person:Person)`.

In a link:{docs-home}/cypher-manual/current/clauses/match/[`MATCH`] query to retrieve all nodes labeled `Person`, this is how it looks like:
...
Note that in the example without a variable, the node `Person` is preceded by a colon (`:`).
