...
[NOTE]
====
It is a best practice to write the relationship type in upper-case, preceded by a colon (`:`).
====

==== Relationship directions

Relationships *always* have a direction and they are expressed in code enclosed by square brackets (`[]`).
...
[CAUTION]
====
Due to performance reasons, it is generally not recommended to query undirected relationships, especially in the case of bigger datasets.
====
...
Considerintg the previous example, another way to organize the same data would be to add Sally and John in the database not as a label of the node type `Person`, but as a node property such as `name`:
...
In case you have already added Sally and John as node labels, but want to change them into node properties, you will need to refactor your graph.
Refactoring is a strategy in link:{docs-home}/model[data modeling] that you can try out in link:{docs-home}/getting-started/data-modeling/graph-model-refactoring/[this tutorial].
...
In order to *do* something with this pattern, such as adding it to the graph or retrieving it in case it already exists, you need to *query* the database.
...
And once this data is written to the database, you can retrieve this pattern:
