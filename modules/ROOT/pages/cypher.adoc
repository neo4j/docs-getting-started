...
Or use a whole word as synonym:
...
One of the benefits of graph databases is that you can create relationships between elements (nodes).
...
When using undirected relationships in a query, Cypher retrieves the relationship with all its connected nodes, no matter the direction of their connection. This allows queries to be flexible and not force the user to know the direction of the relationships stored in the database.
...
Relationship types can categorize and add meaning to a relationship, similarly to how labels group nodes.
...
Also, this makes the query closer to natural language, as it can look like a sentence with a subject (node), a verb (relationship), and an object (another node).
...
When returning it, you don't need to write the labels over again, but use the variables you appended to them (i.e. `p`, `r`, and `t`.).
...
In Cypher, they are represented with curly braces within the parentheses of a node or the brackets of a relationship:
...
Now that the basic Cypher concepts are presented, you can follow this introductory tutorial on how to xref:tutorials/cypher-basics.adoc[Get started with Cypher] for a more hands-on approach.
...
* xref:tutorials/relational-to-graph-import.adoc[Import: RDBMS to graph]
* xref:tutorials/import-relational-and-etl.adoc[Tutorial: Import data from a relational database into Neo4j]
* link:how-to/relational-to-graph-modeling.adoc[Modeling: relational to graph]
...
The link:https://graphacademy.neo4j.com/courses/cypher-fundamentals/[Cypher Fundamentals] course offers further guidance on how to get started with Cypher and writing queries.
