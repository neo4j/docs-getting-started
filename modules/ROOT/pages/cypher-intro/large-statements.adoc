...
In the following example, you first find the collection of movies someone appeared in and then filter out those which appear in only one movie.
...
Only those variables are to be passed to the next part of the query.
...
In the first query, the `Person` name and a collected list of the `Technology` types are being passed.
Therefore only these items can be referenced in the `RETURN` clause.
You cannot use the relationship (`r`) or even the `Person` birthdate because you did not pass those values along.

In the second query, you can reference `p` and any of its properties (`name`, `birthdate`, `yearsExperience`, `twitter`), the collection of friends (as a whole, not each value), and the number of friend-of-friends.
Since you passed those values in the `WITH` clause, you can use those in `WHERE` or `RETURN` clauses.

`WITH` requires all values passed to have a variable (if they do not already have one).
The `Person` nodes are given a variable (`p`) in the `MATCH` clause, so you do not need to assign a variable there.
