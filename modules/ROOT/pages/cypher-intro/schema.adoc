...
Learn more about indexes in link:{neo4j-docs-base-uri}/cypher-manual/4.2/administration/indexes-for-full-text-search#administration-indexes-fulltext-search[Cypher Manual -> Indexes].

[NOTE]
====
It is possible to specify which index to use in a particular query, using _index hints_.
This is one of several options for query tuning, described in detail in link:{neo4j-docs-base-uri}/cypher-manual/4.2/query-tuning#query-tuning[Cypher manual -> Query tuning].
====
...
Learn more about constraints in link:{neo4j-docs-base-uri}/cypher-manual/4.2/administration/constraints#administration-constraints[Cypher manual -> Constraints].
