...
----
+CREATE CONSTRAINT ON (movie:Movie) ASSERT movie.title IS UNIQUE+ label:deprecated[]
----
====
...
You can inspect a database to find out what constraints are defined.

Check the defined constraints with:
...
+----------------------------------------------------------------------------------------------------+
| 4  | "constraint_751eb8ab" | "UNIQUENESS" | "NODE"     | ["Movie"]     | ["title"]  | 3            |
+----------------------------------------------------------------------------------------------------+
----
