...
All of our code examples will continue with the graph example we have been using before, but include some more data for our later queries.
...
At the time Neo4j offers the following types of subqueries:
...
In this section, `EXISTS`, `COUNT`, and `CALL {...}` subqueries are covered.
To learn more about using `CALL {...} IN TRANSACTIONS`, see code examples in the following tutorials on how to import CSV data into Neo4j instance:
...
As you see, subqueries are demarcated by the braces.
...
If you run this query in the Neo4j Browser, the following graph is returned:
...
Let's next write a subquery that does more powerful filtering than what you can achieve with the `WHERE` clause or `exists` function alone.

Assume that: 

* You want to find people who work for a company which name starts with 'Company' and like at least one technology that's liked by three or more people.
...
You can find people that like a technology, but you cannot check that at least three people like that technology as well, because the variable `t` isn't in the scope of the `WHERE` clause.
...
He's been filtered out in this query because the only `Technology` that he likes is *Python*, and there aren't three people who like Python.
...
You can get some of the way there using the `UNION` clause and `COUNT` subquery expression:
...
You could extend our query further to return the technologies that these people like, and the friends that they have.
