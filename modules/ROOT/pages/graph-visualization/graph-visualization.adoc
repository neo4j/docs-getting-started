...
You can also use the built-in drop-down buttons on query result panes to easily export the data to link:https://neo4j.com/docs/browser/[PNG, SVG, or CSV formats].
...
=== Neo4j Python Graph Visualization Library

The Neo4j Python Graph Visualization Library is a Python library that provides tools for visualizing graph data.
It is built on top of the Neo4j Visualization Library and provides a Python interface for creating graph visualizations.
For more information about the Neo4j Python Graph Visualization Library, see the link:{docs-home}/python-graph-visualization/current/[documentation].
