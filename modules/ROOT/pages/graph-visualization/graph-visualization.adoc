...
Bloom is a standalone product focused on visualization that comes with every link:{aura_signup}[AuraDB Instance], and is available both with Neo4j Desktop and standalone.
...
NVL is a collection of libraries you can use if you want to build your own graph visualizations.
The same visualizations are used in Neeo4j Bloom en Explore.
