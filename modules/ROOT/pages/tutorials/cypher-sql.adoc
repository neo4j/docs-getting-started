= Tutorial: Comparing Cypher with SQL
...
This tutorial shows anyone familiar with SQL how to understand Cypher and write the equivalent Cypher queries using the Northwind dataset.

[NOTE]
====
To learn more about the differences between the two query languages, refer to the link:{docs-home}/cypher-manual/current/introduction/cypher-overview/#_cypher_and_sql_key_differences[Cypher documentation].
====

[#cypher-sql-northwind]
== Load the Northwind dataset

The Northwind dataset represents the data storage of a retail application.
It contains customers, products, orders, employees, shippers, and categories as well as their interactions.
Follow these steps to start working with this dataset:

. link:{docs-home}/deployment-options[Create a Neo4j instance].
. Import the Northwind dataset.
.. If you are using Aura, click on the hat icon on the top-right side menu and follow the _Query fundamentals_ guide up to step 2.
+
image::northwind_dataset_load.png[]
.. If you are using Neo4j Browser, run `:guide northwind` in the query area.
+
image::browser-northwind.png[]
.. Another option, functional both in Aura and Browser, is to xref:tutorials/import-relational-and-etl.adoc[import the dataset as a _.zip_ file].
...
A first basic query to do is to find and return records.
...
On the other hand, graph databases store data in a graph with nodes and relationships between them.
...
It is recommended to familiarize yourself with the guides on xref:data-modeling/index.adoc[data modeling] and link:{docs-home}/import[data importing] into Neo4j.
...
If you want to match quickly by a specific node label and attribute combination, it makes sense to create an index for that pair, if you haven't already done that during the xref:tutorials/import-relational-and-etl.adoc[import].
